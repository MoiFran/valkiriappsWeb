...
/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unused-expressions, react-hooks/exhaustive-deps */
...
export interface BallpitProps {
  className?: string;
  /**
   * Number of spheres in the ball pit
   * @default 200
   */
  count?: number;
  /**
  ...
  friction?: number;
  /**
   * Wall bounce coefficient (0-1)
   * @default 0.95
   */
  wallBounce?: number;
  /**
   * Enable cursor following behavior
   * @default true
   */
  followCursor?: boolean;
  /**
   * Array of hex colors for gradient
   * @example [0xff0000, 0x00ff00, 0x0000ff]
   */
  colors?: number[];
  /**
   * Minimum sphere size
  ...
  maxSize?: number;
  /**
   * Material parameters (metalness, roughness, clearcoat, clearcoatRoughness)
   */
  materialParams?: {
    metalness?: number;
    roughness?: number;
...
const Ballpit: React.FC<BallpitProps> = ({ className = "", followCursor = true, ...props }) => {
  ...
  return (
    <canvas
      ...
      ref={canvasRef}
      aria-label="Interactive 3D ball pit animation"
    />
  );
};
